...
module.exports = class Plugin {
    ...
    async model() {
        ...
        this.context._modelCache ??= new Map()
        const cacheKey = JSON.stringify({ files, options })
        if (!this.context._modelCache.has(cacheKey)) {
            const _load = cds.load(files, options)
            this.context._modelCache.set(cacheKey, _load)
        }
        return this.context._modelCache.get(cacheKey)
    }
    ...
    async baseModel() {
        ...
        this.context._modelCache ??= new Map()
        const cacheKey = JSON.stringify({ files: sources.base, options })
        if (!this.context._modelCache.has(cacheKey)) {
            const _load = cds.load(sources.base, options)
            this.context._modelCache.set(cacheKey, _load)
        }
        return this.context._modelCache.get(cacheKey)
    }
